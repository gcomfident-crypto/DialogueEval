...
def scene_asset_prompt(
    ...
    return f"""请基于下面的外呼任务评测标准和业务配置，生成可执行的场景资产。
...
def materialize_eval_standard_prompt(
    ...
    return f"""请把下面的外呼任务评测标准中的模板占位变量实例化为固定业务值。
...
def coverage_plan_prompt(
    ...
    return f"""请为该外呼任务生成覆盖计划 coverage plan。
...
def scoring_rubric_prompt(
    ...
    return f"""请基于外呼任务评测标准生成可执行的评分量表 scoring rubric。
...
def agent_turn_prompt(
    ...
    return f"""你是外呼任务中的客服模型。请根据任务指令生成下一句客服回复。
...
def case_evaluation_prompt(
    ...
    return f"""你是严谨的外呼对话评测员。请基于评分量表和完整对话，对单个 case 做可解释、可量化评分。
...
评分量表：
{json_text(scoring_rubric)}

业务配置：
{json_text(business_config)}

单 case 对话结果：
{json_text(conversation_result)}
...
def coverage_judge_prompt(
    ...
    return f"""你是覆盖率判定器。请根据 coverage plan 的自然语言定义和证据要求，判断当前对话已经触发哪些目标。
...
当前已触发目标：
{json_text(current_triggered_targets)}
